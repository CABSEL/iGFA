...
class parse_model(parse_model_generic):
    ...
    def fetch_data(self, vcd_col='VCD (1E6 VC/mL)', titer_col='Titer (g/L)', spec_prod_col='Spec Prod (pg/cells/day)'):
    ...
    def create_pyomomodel(self, fit_beta=True, regularize_params=True):
        ...
        @model.Expression()
        def min_params_flux(m):
            return (sum((m.alpha[i, j] - 1)**2 for i in m.timepoints for j in m.enzymes) +
                    # sum(abs(m.beta[i, j] - 1) for i in m.timepoints for j in m.beta_mets) +
                    sum((m.gamma[i] - 1)**2 for i in m.timepoints))
        ...
        @model.Constraint()
        def bilevel_constraint(m):
            return (m.fit_flux) <= m.bilevel_weight
        model.bilevel_constraint.deactivate()
        ...
        def bilevel_obj(m):
            added_objective = m.fit_betas if fit_beta is True else 0
            return (m.min_params_flux + added_objective + m.fit_spec_prod)
        model.bilevel_obj.deactivate()

        return model

    def get_results(self, instance):
        results = {}
        results['objectives'] = {e: pyo.value(instance.find_component(e)) for e in ['fit_betas', 'fit_flux', 'fit_spec_prod', 'obj', 'min_params', 'bilevel_constraint', 'bilevel_obj']}  #
